...
## Лицензия (гибридная модель)

- **Этот репозиторий — документация и проектирование — под [GNU AGPL-3.0](LICENSE).**
- **Разрабатываемый код панели — закрытый проприетарный** (приватный репозиторий, не публикуется здесь).

AGPL на документации не связывает код: авторское право защищает выражение (текст/диаграммы), а не идеи и
архитектуру. Поскольку код закрытый, для него обязательны: **clean-room** (не копировать GPL-код
зависимостей в бинарь) и **xray как отдельный процесс** за gRPC (Anti-Corruption Layer). Полный разбор —
[docs/Arch/08-licensing-compliance.md](docs/Arch/08-licensing-compliance.md) §8.5.
